    ...
    useEffect(() => {
        const fetchNewsDetail = async () => {
            try {
                setIsLoading(true)
                setError(null)
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/news?slug=${params.slug}`)
                if (!response.ok) {
                ...
                setError(err instanceof Error ? err.message : 'Yangilik yuklanishida xatolik')
